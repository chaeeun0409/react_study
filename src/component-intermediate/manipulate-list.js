...
const ListItem = function ({ onRemove, onUpdate, item: { id, value } }) {
    ...
    return (
        <div style={{
            border: '1px solid black', margin: '20px', padding: '20px'}}>
            <span>id : {id}</span><br />
            <span>value : {value}</span><br />
            {/* 화살표 함수를 전달하여, 부모로 부터 전달받은 상태 변경 메소드(onRemove,
onUpdate)를 호출 */}
            <button onClick={() => onRemove(id) /* 필요한 인자값(삭제할 id) 전달
*/}>remove</button>&nbsp;
            <button onClick={() => onUpdate(id, { id: id, value: value + 1 }) /*
필요한 인자값(추가할 객체) 전달 */}>update</button>
        </div>
    )
}
const ListComponent = function (props) {
    ...
    const handleCreateItem = () => setList(state => state.concat({
        id: v4(),
        value: 0
    }))
    // remove (filter)
    // id가 일치할 경우 false 리턴하여 리스트에서 제외
    const handleRemoveItem = (uuid) => setList(state => state.filter((item) =>
        item.id !== uuid))
    // update (map)
